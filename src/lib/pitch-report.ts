...
function hoaChecklistSlide(): string {
  ...
  return `<div class="slide"><div class="pad">${logoBadge()}<div class="section-title">Operational Tools</div><div style="display:grid;grid-template-columns:1.08fr .92fr;gap:18px;align-items:stretch"><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">${tools.map(([icon, title, items]) => `<div class="gold-card" style="padding:14px 16px;min-height:188px"><div style="display:flex;gap:10px;align-items:center;margin-bottom:8px"><div style="width:42px;height:42px;border-radius:10px;background:#34444f;color:#B8973A;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;letter-spacing:.7px">${icon}</div><div class="sub-heading" style="font-size:18px;margin:0">${title}</div></div>${hoaBulletList(items as string[])}</div>`).join('')}</div><div><div class="visual-card" style="padding:18px;margin-bottom:14px;background:linear-gradient(135deg,#34444f,#1a2744);color:#fff"><div class="sub-heading" style="font-size:20px;color:#B8973A">Operating Command Center</div><p class="small" style="color:rgba(255,255,255,.84);margin-bottom:13px">Each tool is meant to turn board concerns into assigned work: visible files, clear questions, photo-backed inspections, vendor accountability, and executive summaries.</p><div style="display:grid;gap:9px">${workflow.map(([num, title, copy]) => `<div style="display:flex;gap:10px;align-items:center;background:rgba(255,255,255,.08);border:1px solid rgba(184,151,58,.32);border-radius:8px;padding:10px"><div style="width:34px;height:34px;border-radius:50%;background:#B8973A;color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900">${num}</div><div><div style="font-size:13px;font-weight:900;color:#fff">${title}</div><div style="font-size:11px;color:rgba(255,255,255,.72)">${copy}</div></div></div>`).join('')}</div></div><div class="visual-card" style="padding:14px"><div class="sub-heading" style="font-size:17px;margin-bottom:8px">Outputs the Board Receives</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">${['Action log', 'Photo report', 'Vendor tracker', 'Claims tracker', 'Board packet', 'Closeout file'].map(item => `<div style="border:1px solid rgba(184,151,58,.28);background:#FAF8F5;border-radius:7px;padding:9px 10px;font-size:12px;font-weight:800;color:#1a2744">${item}</div>`).join('')}</div></div></div></div><div class="source-note">Additional deliverables: one-page sheet, service menu, Why Camelot page, claims workflow, vendor workflow, cover letter.</div></div></div>`;
}
